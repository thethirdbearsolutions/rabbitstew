"""What do the foraging ecology's bests do?  Alone in the arena on fresh seeds: items eaten, displacement,
and the same with the food sensors blanked, all env sensors blanked, or the local brains silenced."""
...
def trial(g, seed, mode):
    ...
    return {"food": float(sim.food_eaten[0]), "disp": float(np.linalg.norm(sim.center_of_mass(0)[:2] - p0)), "path": path, "work": float(sim.work[0]) / 1000}
for kind in ("holistic", "conventional"):
    for gen in gens:
        ...
        print("     " + "  ".join(f"{m}: food {row[m]['food']:.2f} disp {row[m]['disp']:.2f} path {row[m]['path']:.1f} work {row[m]['work']:.1f}kJ" for m in row), flush=True)
