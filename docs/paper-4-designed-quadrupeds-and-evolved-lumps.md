...
**Neighbour links are reachable, not just permitted.** A control from the Sims-budget package (Chaotic RBT-5, `docs/sims-budget-run.md`): in a three-generation population-300 run with `--neighbour-links`, 15,484 of 31,415 local brain links in the final holistic population read a neighbouring part's units; in the same run with the flag omitted, 0 of 219. Whether any of those links ever matters to a working path is what RBT-11 and RBT-5 are running to find out; every labbed champion so far was evolved without them. The first twenty generations of the seed-902 Sims-budget run (RBT-5) then showed what they carry: the champion at generation 20 is three spheres and four effectors with every sensor unlinked, and the links that move it run between parts (an effector on the ball-jointed sphere driving an effector on the root at +3.51; lesioning it stops the robot dead). Sims' encoding distributed a motor pattern on its first outing, not a sensorimotor loop; and the champion's fresh time-at-target of 0.21, above the competence threshold with no sensor wired, is the case the lab rule exists to catch.

**The Sims budget (RBT-11, seed 901, 150 generations).** Population 300 in each fauna, survival selection, lexicase over the score vector, mirrored connections, neighbour links, a 100-generation heading curriculum, solo dense score throughout: fifteen times the evaluations per generation of the capacity runs, on a four-core box over eight hours. It bought what population 20 bought and no more. The final holistic best's fresh time at target is 0.20 pooled over 44 unseen draws, on the threshold and flat since generation 90, and it steers to one bearing of three (the one behind it); the Pioneer's controller reached 0.73 and all three. The winning lineage was two parts at generation 0 and two parts at 149, carried no joint sensor at any checkpoint, and its controller is A-301's bearing-in-own-frame reflex read from a second frame: the neighbour links are on the working path for the first time in the series (ten cross-part links in every best; cutting them alone takes time at target from 0.20 to 0.02), and what they carry is the same reflex twice, overlapping rather than combining (the whole local-brain lesion costs 0.17, the two single-part lesions sum to 0.26). The global brain mattered at generations 50 and 100 of this lineage and not at 149, so "the central brain is always inert" holds for the champions and not for every ancestor of one. Two numbers contradict the earlier papers and are taken as corrections: the holistic solo-score heritability is 0.08 to 0.11 by a newborn-only estimator (survival re-logs each surviving parent every generation and the standard tool's 0.31 in the middle window is survivors, not inheritance), against paper 3's 0.35 to 0.41 at population 20; and both faunas collapsed to three of three hundred founders, for which the tool's noise null (tournament with elites) is not the right model, so drift and selection cannot be told apart from the count. Report: `runs/RBT-11/REPORT.md` on branch `claude/dazzling-lamport-y47veb`.
