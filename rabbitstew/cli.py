...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--breed-stream", type=int, default=None, metavar="K", help="the replicate history (RBT-105): the holistic founders and their ages are drawn from --seed exactly as without this flag, then the holistic fauna's stream is replaced by an independent replicate K (>= 1) for everything after (groupings, breeding, mutation); the designed-body fauna and the worlds are untouched. 0 is the original stream")
    s.add_argument("--workers", type=int, default=1)
    s.add_argument("--seed", type=int, default=0)
    s.add_argument("--duration", type=float, default=None)
    s.add_argument("--size-ratio", type=float, default=None)
