...
def _add_food_args(s) -> None:
    ...
    s.add_argument("--smell", choices=["sum", "mean", "log"], default="sum", help="how the smell sensors combine their sources: sum (saturates at high density), mean or log (normalised, so the gradient survives)")
...
def cmd_evolve(args) -> int:
    ...
    cfg = EvolutionConfig(
        ...
        morph_protection=args.protect_morphology,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--living-cost", type=_cost, default=0.05, help="energy per season just to exist; 'relative' (each population's mean gain that season) is retired because it conserves energy and stops a converged population breeding")
    ...
    s.add_argument("--link-scale", type=float, default=1.0, metavar="K", help="RBT-104: the scale of the designed body's link-weight space: its founders' link weights are multiplied by K at founding, and every link-weight draw of its controller operator (a new link's N(0,1), a reset's N(0,1), a step's N(0,weight_sigma)) by K; unit biases and the holistic fauna are untouched and no extra random number is drawn. 1.0 (the default) is the run as it was, byte for byte")
