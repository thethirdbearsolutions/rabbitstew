...
## 7a. Holding the path *length* flat is not enough: shape moves the rate by 2.07x

Filed after RBT-67 read a rise in items per in-disc metre "on in-disc path that is flat from a = 64
up" as steering rather than coverage. Pinning the length is a real repair — it removes the confound
that a controller which merely moves more clears the rate — but it is not sufficient, because a robot
that re-covers ground it has already eaten meets less food per metre than one on fresh ground.

Same body, same 200 world-dealt layouts, same eat rule, four paths of **identical** length (4.28 m,
this champion's own in-disc path), and **no sensing anywhere in it**
(`shape_at_constant_length.py` / `.txt`):

| path shape, all 4.28 m | items per in-disc metre |
|---|---|
| tight zigzag, partly re-covering | 0.703 |
| straight line, fresh ground | 0.669 |
| one tight circle, retraces itself | 0.518 |
| out and back over its own track | **0.340** |

**A factor of 2.07 from shape alone, at pinned length and zero steering.** So a rise in items per
metre on flat path is a signature a gain that merely straightened the sweeps would also produce. For
scale, RBT-67's rises are x2.6 (W4b-801) and x2.9 (P-801) — larger than this spread but the same
order, so the leg is not empty and is not decisive either. Its conclusion is safe regardless, on its
phantom-smell control at a = 384, which is trajectory-independent in the way items per metre is not.

The shapes here are deliberately extreme and are on a different substrate, so 2.07 is the existence
and rough size of the confound rather than a correction to anyone's figures. The comparison immune to
both length and shape is the trajectory null itself.
