#!/bin/bash
# RBT-100: smoke test of the C3 pipeline (RBT-92's tables.py and cull_k.py, runs/RBT-100/readout.py) on the
...
grep -E '^[A-Z][A-Z0-9 -]+|^  V[0-3]|^  C3-V1|FAIL|CLASS|UNDERSIZED|not undersized|echo' "$S/readout.txt" | sed -E 's/[-+]?[0-9]+\.[0-9]+/#/g; s/\[[^]]*\]/[...]/g'
