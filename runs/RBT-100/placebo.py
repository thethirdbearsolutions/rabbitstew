"""RBT-100 (C3): the coordinator's 18:45 ruling on RBT-92's readout adversary, applied to C3 without changing the
registered readout.  RBT-99's placebo.py with C3's paths and C3's D count; nothing else changed.  Method: RBT-92's readout adversary, probe_readout.py P3/P4 (PR #183,
runs/RBT-92/readout-adversary/), reused through readout.py's own Arm / rbody / classify / stat, committed tables only.
  ...
  sign guard margin, and the per-seed RMS of the A/A-like references (R-cull20 recovery, both faunas)
    ...
    python runs/RBT-100/placebo.py > runs/RBT-100/placebo.txt
"""
import math
import os
import statistics
...
K = {}
for s in SEEDS:
    for l in open(f"runs/RBT-100/cull-k-{s}.txt"):
        if l.startswith("cull\t"):
            K[s] = {p.split("=")[0]: int(p.split("=")[1]) for p in l.split("\t")[1].strip().split(",")}


def path(arm, s):
    return {"base": f"runs/RBT-90/forage-{s}", "shift": f"runs/RBT-100/shift-{s}", "cull": f"runs/RBT-100/cull-{s}",
...
for a, dd in (("base", 0), ("shift", 7)):  # the shift arm's D count, as the readout prints it (7/10)
