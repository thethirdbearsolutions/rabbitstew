...
def main(root, seeds, write=False, from_summaries=False):
    ...
    if ab:
        ...
        print(f"the +-0.10 rule against the A/A null: a 4-seed mean must clear {T4 * se0:.4f} to be outside the null at 95%; 0.10 is {0.10 / se0:.2f} null SEs (noise alone {'meets' if 0.10 < T4 * se0 else 'does not meet'} |mean| >= 0.10 more than 1 time in 20: P(|mean| >= 0.10 | null) = {t_sf2(0.10 / se0, 4):.3f})")
