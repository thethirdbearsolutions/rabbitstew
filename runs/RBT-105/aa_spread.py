"""RBT-105: the ecology's founder-sharing A/A spread, REPORT ONLY (the design adversary's F4, PR #154; adopted by the coordinator's ruling, item 7).
...
window this is an upper bound on the challenge arms' own A/A spread, not an estimate of it.
    ...
    python runs/RBT-105/aa_spread.py [ARMS_DIR]  > runs/RBT-105/aa_spread.txt
"""
import csv
import math
import os
...
T = onsets()
print("RBT-105 ecology A/A spread: REPORT ONLY; it rules on nothing.")
print("CAVEAT: these replicates share founders, worlds and the designed-body fauna, but their holistic history diverges from")
print("season 0; a challenge arm (RBT-92/99/100/101) shares its baseline up to its onset T and diverges after. For the recovery")
print("window [T+60, T+160) this spread is therefore an UPPER BOUND on a challenge arm's own A/A spread, not an estimate of it.")
print("A matched A/A would switch the stream at T (a later flag, not RBT-105's). Conventional quantities are omitted: identical by construction.\n")
