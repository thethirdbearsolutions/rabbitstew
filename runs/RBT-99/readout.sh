...
#
#   shift-SEED, cull-SEED     runs/RBT-99/  (this ticket's arms, and cull-k-SEED.txt, which V1 reads)
...
#
# After readout.py, runs/RBT-99/c2_block.py prints C2's block: the mechanical price beside R-shift, turnover
# per window, the null note and the C2 framing (Amendment 1).  readout.sh exits with readout.py's status.
set -e
S=$(mktemp -d)
trap 'rm -rf "$S"' EXIT
cd "$(dirname "$0")/../.."   # the repository root, wherever it is launched from
R=$(pwd)
for SEED in ${RBT92_SEEDS:-801 804 805 806 807 1 2 3 4 7}; do
  ...
  # the cull's k file too: readout.py's V1 and its k = 0/0 rule read it from the arm directory
  [ -e "$R/runs/RBT-99/cull-k-$SEED.txt" ] && ln -s "$R/runs/RBT-99/cull-k-$SEED.txt" "$S/cull-k-$SEED.txt"
...
status=$?
set -e
# C2's block (price, turnover, null note, framing): PREREGISTRATION.md Amendment 1, answering F4-F7
RBT92_ONSET="${RBT92_ONSET:-$R/runs/RBT-92/onset.txt}" python "$R/runs/RBT-99/c2_block.py"
exit $status
