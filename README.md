...
A seventh, from the arena: **when the score is a contest, the opponent is a covariate, and it is stated before the run.** RBT-74 paired protected and unprotected arena runs by seed and the pairing was harmful, not merely void: paired SE 0.073 against unpaired 0.063, correlation −0.41 across four seeds, and which runs happened to draw a runaway wheeled opponent predicted 103% of the headline difference. Two things follow. An arena `Experiment` now draws from three generators derived independently from its seed (`spawn_streams`: one per population, one for the terrain and start layouts), so an arm that changes one population's reproduction meets the same other population on the same terrains, and `tests/test_rng_streams.py` pins it: two runs at one seed that differ only in `--protect-morphology` write byte-identical conventional `lineage.jsonl` lines and terrain seeds. **Arena runs made before RBT-85 drew everything from one stream and do not reproduce from their configs under this code; their `state.json` checkpoints are refused on resume rather than continued under different draws.** The ecology gained the same three streams under RBT-95, with one limit stated in `runs/README.md`: after a merge the comparator's income is a contest outcome and stays a covariate. And shared is not constant: whether a wheeled population becomes a runaway is still a coin flip, now one both arms share, so any arena arm's pre-registration names the wheeled side's final-fifth solo approach (from `rabbitstew analyze`) as a covariate, and the report prints it beside every paired difference whether or not it helps (RBT-74, RBT-85).
